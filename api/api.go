...
func CreateEmployeeData(c *gin.Context) {
	...
	if request.AnnualPackage != "" {
		if salaryValue, err := strconv.ParseFloat(request.AnnualPackage, 32); err == nil && salaryValue > 0 {
			salary = float32(salaryValue)
		}
	}

	if err := scyllaClient.Query(salaryQueryString,
		request.ID, processDate, request.Name, salary, request.Status).Exec(); err != nil {
		logrus.Warnf("Failed to create salary record for employee %s: %v", request.ID, err)
		// Don't fail the whole operation, just log the warning
	} else {
		logrus.Infof("Successfully created salary record for employee %s with salary %.2f", request.ID, salary)
	}
	...
	if redisEnabled {
		client.CreateRedisClient().HDel(ctx, "employee", "all_data")
